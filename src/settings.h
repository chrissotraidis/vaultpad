...
struct InputSettings {
    // hybrid: direct touch in gameplay and UI, preserving trackpad contexts.
    // trackpad: relative cursor movement everywhere.
    // touch: direct touch in every context.
    std::string touch_mode = "hybrid";
    double touch_sensitivity = 1.0;

    bool isTrackpad() const { return touch_mode == "trackpad"; }
    bool isTouchEverywhere() const { return touch_mode == "touch"; }
};
