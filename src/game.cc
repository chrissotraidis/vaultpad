...
static bool gSavedForCurrentBackground = false;
...
void gameHandleAppBackgrounding()
{
    settingsSave();

    if (gSavedForCurrentBackground || gDude == nullptr || gMapHeader.name[0] == '\0') {
        return;
    }

    constexpr int safeModes = GameMode::kWorldmap | GameMode::kCombat | GameMode::kPlayerTurn;
    if ((GameMode::getCurrentGameMode() & ~safeModes) != 0 || settings.ui.auto_quick_save <= 0) {
        debugPrint("Background autosave skipped because the current game mode is not save-safe.\n");
        return;
    }

    int rc = lsgSaveGame(LOAD_SAVE_MODE_QUICK);
    if (rc == 1) {
        gSavedForCurrentBackground = true;
        debugPrint("Background autosave completed.\n");
    } else {
        debugPrint("Background autosave failed.\n");
    }
}

void gameHandleAppForegrounding()
{
    gSavedForCurrentBackground = false;
}
...
static void TryLoadBaseCEMod()
{
    ...
    char* bundlePath = SDL_GetBasePath();
    if (bundlePath != nullptr) {
        char absolutePath[COMPAT_MAX_PATH];
        snprintf(absolutePath, sizeof(absolutePath), "%sce.dat", bundlePath);
        bool loaded = tryLoadBaseCEModAtPath(absolutePath, &found, &openFailed);
        SDL_free(bundlePath);
        if (loaded) {
            return;
        }
...
void gameHandleSkilldexResult(int rc)
{
    ...
    const char* touchConfirmation = nullptr;
#endif
...
#ifdef FALLOUT_IOS_PRODUCT_BOOTSTRAP
    if (touchConfirmation != nullptr) {
        displayMonitorAddMessage(touchConfirmation);
    }
#endif
}
