...
static void mouseDeviceMapWindowToLogicalPosition(int* x, int* y)
{
    if (gSdlRenderer != nullptr) {
        float logicalX;
        float logicalY;
        SDL_RenderWindowToLogical(gSdlRenderer, *x, *y, &logicalX, &logicalY);
...
void mouseDeviceRefreshWindowMapping()
{
    ...
    // SDL's renderer transform handles logical scaling, DPI, and letterboxing.
    // Keep these cached dimensions as a fallback before the renderer exists.
