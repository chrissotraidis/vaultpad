...
static bool _setup_inventory(int inventoryWindowType)
{
    ...
    if (doneButtonXY.first != -1) {
        ...
        // The original button only covers the tiny red lamp at the right edge
        // of the visible DONE control. Make the complete label a touch target.
        int doneTouchButton = buttonCreate(gInventoryWindow,
            doneButtonXY.first - 92, doneButtonXY.second - 16,
            112, 44,
            -1, -1, -1, KEY_ESCAPE,
            nullptr, nullptr, nullptr, BUTTON_FLAG_TRANSPARENT);
        if (doneTouchButton != -1) {
            buttonSetCallbacks(doneTouchButton, _gsound_red_butt_press, _gsound_red_butt_release);
        }
#endif
    }
...
// Dragging between the four barter columns is natural with a mouse, but on a
// touchscreen a tap has no distinct drop location. Treat a tap as a transfer
// to the matching offer/inventory column on iOS. Stacks still use the quantity
// picker; desktop keeps the original drag and Ctrl-transfer behavior.
static bool barterUsesTapTransfer()
{
#if defined(__APPLE__) && TARGET_OS_IOS
    return true;
#else
...
static void barterMoveToTable(Object* item, int quantity, int slotIndex, int indexOffset, Object* npc, Object* sourceTable, bool fromDude)
{
    ...
    bool immediate = _ctrl_pressed();
    bool tapTransfer = barterUsesTapTransfer();
    _drag_item_loop(item, immediate || tapTransfer);

    if (fromDude) {
        if (immediate || tapTransfer || mouseHitTestInWindow(gInventoryWindow, INVENTORY_TRADE_INNER_LEFT_SCROLLER_TRACKING_X, INVENTORY_TRADE_INNER_LEFT_SCROLLER_TRACKING_Y, INVENTORY_TRADE_INNER_LEFT_SCROLLER_TRACKING_MAX_X, INVENTORY_SLOT_HEIGHT * gInventorySlotsCount + INVENTORY_TRADE_INNER_LEFT_SCROLLER_TRACKING_Y)) {
        ...
        if (immediate || tapTransfer || mouseHitTestInWindow(gInventoryWindow, INVENTORY_TRADE_INNER_RIGHT_SCROLLER_TRACKING_X, INVENTORY_TRADE_INNER_RIGHT_SCROLLER_TRACKING_Y, INVENTORY_TRADE_INNER_RIGHT_SCROLLER_TRACKING_MAX_X, INVENTORY_SLOT_HEIGHT * gInventorySlotsCount + INVENTORY_TRADE_INNER_RIGHT_SCROLLER_TRACKING_Y)) {
...
static void barterMoveFromTable(Object* item, int quantity, int slotIndex, Object* npc, Object* sourceTable, bool fromDude)
{
    ...
    if (fromDude) {
        if (immediate || tapTransfer || mouseHitTestInWindow(gInventoryWindow, INVENTORY_TRADE_LEFT_SCROLLER_TRACKING_X, INVENTORY_TRADE_LEFT_SCROLLER_TRACKING_Y, INVENTORY_TRADE_LEFT_SCROLLER_TRACKING_MAX_X, INVENTORY_SLOT_HEIGHT * gInventorySlotsCount + INVENTORY_TRADE_LEFT_SCROLLER_TRACKING_Y)) {
        ...
        if (immediate || tapTransfer || mouseHitTestInWindow(gInventoryWindow, INVENTORY_TRADE_RIGHT_SCROLLER_TRACKING_X, INVENTORY_TRADE_RIGHT_SCROLLER_TRACKING_Y, INVENTORY_TRADE_RIGHT_SCROLLER_TRACKING_MAX_X, INVENTORY_SLOT_HEIGHT * gInventorySlotsCount + INVENTORY_TRADE_RIGHT_SCROLLER_TRACKING_Y)) {
...
static int inventoryQuantityWindowInit(int inventoryWindowType, Object* item)
{
...
#if defined(__APPLE__) && TARGET_OS_IOS
    // The art places the visible words beside tiny red-lamp buttons. Cover the
    // complete labels so confirming or cancelling a typed quantity is reliable.
    int doneTouchButton = buttonCreate(_mt_wid,
        18, 112, 100, 44,
        -1, -1, -1, 500,
        nullptr, nullptr, nullptr, BUTTON_FLAG_TRANSPARENT);
    if (doneTouchButton != -1) {
    ...
    int cancelTouchButton = buttonCreate(_mt_wid,
        138, 112, 105, 44,
        -1, -1, -1, KEY_ESCAPE,
        nullptr, nullptr, nullptr, BUTTON_FLAG_TRANSPARENT);
    if (cancelTouchButton != -1) {
        buttonSetCallbacks(cancelTouchButton, _gsound_red_butt_press, _gsound_red_butt_release);
    }
#endif
