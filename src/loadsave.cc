...
static int lsgWindowInit(int windowType)
{
    ...
    // The original hit regions cover only the tiny red lamps, not the visible
    // DONE/CANCEL labels. Add transparent iPad-sized targets over each complete
    // control while retaining the original artwork and callbacks.
    btn = buttonCreate(gLoadSaveWindow, 380, 335, 105, 44, -1, -1, -1, kLoadSaveActionDone, nullptr, nullptr, nullptr, BUTTON_FLAG_TRANSPARENT);
    if (btn != -1) {
        buttonSetCallbacks(btn, _gsound_red_butt_press, _gsound_red_butt_release);
    }

    btn = buttonCreate(gLoadSaveWindow, 488, 335, 112, 44, -1, -1, -1, 501, nullptr, nullptr, nullptr, BUTTON_FLAG_TRANSPARENT);
    if (btn != -1) {
        buttonSetCallbacks(btn, _gsound_red_butt_press, _gsound_red_butt_release);
    }
#endif
