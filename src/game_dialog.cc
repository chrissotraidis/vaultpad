...
static int createDialogTouchTarget(int win, int x, int y, int width, int height, int keyCode, void (*mouseUp)(int, int) = nullptr)
{
    int h = buttonCreate(win, x, y, width, height,
        -1, -1, -1, keyCode,
...
#if defined(__APPLE__) && TARGET_OS_IOS
    // The original controls only expose their tiny red lamps as buttons.
    // Make the complete TRADE and TALK labels comfortable touch targets.
    int tradeTouchBtn = createDialogTouchTarget(win.get(), 28, 130, 120, 60, KEY_LOWERCASE_M);
    if (tradeTouchBtn == -1) return -1;
    int talkTouchBtn = createDialogTouchTarget(win.get(), 494, 130, 120, 60, KEY_LOWERCASE_T);
    if (talkTouchBtn == -1) return -1;
#endif
...
#if defined(__APPLE__) && TARGET_OS_IOS
    _gdialog_buttons[2] = tradeTouchBtn;
    _gdialog_buttons[3] = talkTouchBtn;
#endif
...
#if defined(__APPLE__) && TARGET_OS_IOS
    // The visible BARTER label sits left of the original tiny lamp.
    int barterTouchBtn = createDialogTouchTarget(win, 494, 4, 120, 70, -1, gameDialogBarterButtonUpMouseUp);
    if (barterTouchBtn == -1) return -1;
#endif
...
#if defined(__APPLE__) && TARGET_OS_IOS
    _gdialog_buttons[3] = barterTouchBtn;
#endif
