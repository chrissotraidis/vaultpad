...
#define MAIN_MENU_TOUCH_ROW_X 16
#define MAIN_MENU_TOUCH_ROW_Y 15
#define MAIN_MENU_TOUCH_ROW_WIDTH 200
#define MAIN_MENU_TOUCH_ROW_HEIGHT 34
...
static int gMainMenuTouchRows[MAIN_MENU_BUTTON_COUNT];
...
static bool mainMenuCreateButtons(const MainMenuLayout& layout, const MainMenuOffsets& offsets)
{
    ...
    for (int index = 0; index < MAIN_MENU_BUTTON_COUNT; index++) {
        Point touchRowPosition = mainMenuTransformPoint(layout, MAIN_MENU_TOUCH_ROW_X, MAIN_MENU_TOUCH_ROW_Y + index * MAIN_MENU_BUTTON_Y_STEP);
        Size touchRowSize = mainMenuTransformSize(layout, MAIN_MENU_TOUCH_ROW_WIDTH, MAIN_MENU_TOUCH_ROW_HEIGHT);
        gMainMenuTouchRows[index] = buttonCreate(gMainMenuWindow,
            touchRowPosition.x + offsets.menuX,
            touchRowPosition.y + offsets.menuY,
            touchRowSize.width,
            touchRowSize.height,
            -1,
            -1,
...
void mainMenuWindowFree()
{
    for (int index = 0; index < MAIN_MENU_BUTTON_COUNT; index++) {
        ...
        if (gMainMenuTouchRows[index] != -1) {
            buttonDestroy(gMainMenuTouchRows[index]);
            gMainMenuTouchRows[index] = -1;
        }
    }
