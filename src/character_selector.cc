...
static bool characterSelectorCreateTouchTargets();
...
static bool characterSelectorCreateTouchTargets()
{
    ...
    static constexpr TouchTarget touchTargets[] = {
        { 96, 315, 120, 32, KEY_LOWERCASE_T },
        { 450, 312, 125, 35, KEY_LOWERCASE_M },
        { 95, 414, 245, 36, KEY_LOWERCASE_C },
        { 476, 414, 110, 36, KEY_ESCAPE },
    };

    for (const TouchTarget& target : touchTargets) {
        if (buttonCreate(gCharacterSelectorWindow,
                target.x,
                target.y,
                target.width,
                target.height,
...
static bool characterSelectorWindowInit()
{
    ...
    if (!characterSelectorCreateTouchTargets()) {
        return characterSelectorWindowFatalError(false);
    }
