...
void _mouse_info()
{
    ...
    // Touch taps synthesize input independently of SDL's mouse state. Direct
    // touch spends one tick positioning, one updating hover, one pressing, and
    ...
    if (touch_get_gesture(&gesture)) {
        ...
        case kTap: {
            ...
            // Toolbar taps bypass the mouse pipeline entirely: the toolbar is
            // a separate, non-overlapping gameplay strip, so it must win over
            // both direct-touch and trackpad world input.
            ...
            if (gesture.numberOfTouches == 1) {
                button = MOUSE_STATE_LEFT_BUTTON_DOWN;
            } else if (gesture.numberOfTouches == 2) {
                button = MOUSE_STATE_RIGHT_BUTTON_DOWN;
            }
