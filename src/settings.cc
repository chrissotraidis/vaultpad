...
static void normalizeTouchMode(std::string& value, const char* section, const char* key)
{
    if (value != "hybrid" && value != "trackpad" && value != "touch") {
        debugPrint("config value %s.%s was reset to hybrid.\n", section, key);
        value = "hybrid";
    }
}
...
#define SECT input
    SETTING_P(touch_mode, normalizeTouchMode);
    SETTING_P(touch_sensitivity, clamp(0.25, 3.0));
#undef SECT
